...
class BasicTypes {
   ...
   * * mold.bool()
   * * mold.bool({params})
   * * mold.bool(predefinedValue, {params})
   */
  bool (firstArgument, secondArgument) {
    return this._generateNode('bool', firstArgument, secondArgument);
  }

  int (firstArgument, secondArgument) {
    return this._generateNode('int', firstArgument, secondArgument);
  }
  ...
  _parseArguments (type, firstArgument, secondArgument) {
    ...
    var isValueCorrectType = _['is' + this._nativeTypeName(type)](firstArgument);
  ...
  _nativeTypeName (shortName) {
    if (shortName === 'bool') return 'Boolean';
    else if (shortName === 'int') return 'Number';
    else if (shortName === 'string') return 'String';
  }
}
